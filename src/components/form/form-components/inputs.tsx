...
function InputWrapper({id, label, children}:{id:string, label:string, children:React.ReactNode}){
    return(
        <div className=''>
            <label htmlFor={id} className='text-sm text-black mb-2 capitalize block'>{label}</label>
            <div className='border border-primary rounded-full p-4 relative'>
                {children}
            </div>
        </div>
    )
...
export function Input({value, handleChange, id, type}:{id: string, type: string, value: string, handleChange: (e: React.ChangeEvent<HTMLInputElement>) =>void}) {
    return (
...
export function Password({value, handleChange, id}:{id: string, value: string, handleChange: (e: React.ChangeEvent<HTMLInputElement>) =>void}) {
    ...
    return (
        <InputWrapper id={id} label={id}>
            <input
                ...
                className='bg-transparent focus:outline-none'
            />
            <button onClick={()=>setShowPassword(!showPassword)} className='absolute top-1/2 -translate-y-1/2 right-4'>
